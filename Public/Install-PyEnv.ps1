function Install-PyEnv {
  ...
  process {
    if ($skip_inst) { return }
    Write-Console "Installing pyenv from https://github.com/pyenv/pyenv-installer" -f LimeGreen
    ...
    if (!$skip_inst) {
      $(Write-Console "TEST: gcm pyenv" -f Yellow; $has_pyenv.Invoke()) ? (Write-Console "Successfully installed pyenv" -f LimeGreen) : (Write-Console "pyenv install failed" -f LightCoral)
    } else {
